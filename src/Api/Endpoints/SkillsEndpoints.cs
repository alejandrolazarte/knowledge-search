...
static class SkillsEndpoints
{
    public static void MapSkillsRoutes(this WebApplication app, string skillsDir)
    {
            ...
            try
            {
                foreach (var dir in Directory.GetDirectories(skillsDir).OrderBy(Path.GetFileName))
                {
                    ...
                    if (exists)
                    {
                        ...
                        catch (Exception ex) { detail = "READ ERR: " + ex.Message; }
                    }
        ...
        app.MapGet("/skills", () =>
        {
            if (!Directory.Exists(skillsDir)) return Results.Ok(new List<SkillSummary>());
    ...
    static (string name, string description) ParseFrontmatter(string content)
    {
        ...
        foreach (var raw in content.Split('\n'))
        {
            ...
            if (line.StartsWith("description:")) description = line[12..].Trim();
        }
